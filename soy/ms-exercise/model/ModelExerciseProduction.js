...
class ModelExerciseProduction extends Model {
  ...
  constructor(data) {
    ...
    //this.submission_date = data.submission_date; <- filled automatically by postgres
  ...
  static async readExeList(ex_id, ps_id) {
    ...
    try {
      ...
      debug("Releasing client from DB pool used to read exercise list")
      client.release();
    }
  }
  ...
  static async getStats(ex_id, ps_id) {
    ...
    try {
      ...
      debug("Releasing client from DB pool used to read stats")
      client.release();
    }
  }
  ...
  static async getBest(ex_id, user_id, ps_id) {
    ...
    try {
      ...
      debug("Releasing client from DB pool used to get best score")
      client.release();
    }
  }
  ...
  static async readStuList(user_id, ps_id) {
    ...
    try {
      ...
      debug("Releasing client from DB pool used to read student's list of exercise productions")
      client.release();
    }
  }
  ...
  static async readStuExList(user_id, ex_id, ps_id) {
    ...
    try {
      ...
      debug("Releasing client from DB pool used to read student's list of exercise productions for one exercise")
      client.release();
    }
  }
  ...
  static async evaluateStudProduction(ps_id, ex_id, user_id, data_file, callback) {
    ...
    ControllerExerciseAPI.getExercise(ex_id)
      ...
      .then((exercise) => {
        ...
        debug("start paths");
        // Chouki: var path below causes a problem when the same user send several times an exercise prod
        // case that happens in load tests or if s/he shares her/his login/password
        // I comment it below and replace it by adding to the path a random part (see below)
        //let path = "/tmp/storage-fs/" + user_id + "/" + ex_id + "/";
        const rndToday = new Date().getTime();
        const rand = Math.random().toString(36).substring(2); //Generate a random identifer
        let path = "/tmp/storage-fs/" + user_id + "/" + ex_id + "/" + rndToday + "/" + rand + "/";
  ...
  static async evaluateStudProductionLMS(ex_id, archRep, lmsStudentStatement, callback) {
    ...
    let exercise = axios
      ...
      .then((exercise) => {
        ...
        const rndToday = new Date().getTime();
        const rand = Math.random().toString(36).substring(2); //Generate a random identifer
        let path = "/tmp/storage-fs/lms-session/" + exercise.ex_id + "/" + rndToday + "/" + rand + "/"; //Shouldn't we remove this from file system when the evaluation is done ?
...
ModelExerciseProduction.keys = [
  ...
  //["submission_date", "timestamptz"], //automatically filled by postgres (if indicated here: not filled !!!)
  ["ps_id", "int"]
];
