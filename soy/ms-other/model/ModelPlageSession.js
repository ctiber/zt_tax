...
class ModelPlageSession extends Model {
  ...
  // We always return 1 to avoid having application error with old data
  // However to be sure to have the desired logic, be sure to update your data (i.e. not 2 sessions have the same seq_id)
  static async readBySeq(seq_id){
    debug("read by seq");
    const client = ModelDB.connect_to_db();
    try {
      const sql = "SELECT * FROM " + this.dbName + " WHERE seq_id = $1;";
      const tab = await client.query(sql, [seq_id]);
      debug("read by seq : SUCCESS");
      return tab.rows[0];
    } catch (err) {
      debug("read by seq : " + err.stack);
      return false;
    } finally {
      client.end();
    }
  ...
  static async verifySubscription(session, user) {
    ...
    if (user) {
      ...
      try {
        ...
        const res = await client.query(sql, [user, session]);
        const sql2 = "SELECT * from session WHERE ps_id = $1::int"
        const res2 = await client.query(sql2, [session])
        debug("readStudentSub : SUCCESS");
        debug("res.rows = ", res.rows);
        return Number(res.rows[0].count) > 0 || res2.rows[0].author === user;
      } catch (err) {
        debug("readStudentSub : " + err.stack);
        return false;
      } finally {
  ...
  static async verifyIfUsedSequence(seq_id) {
    const client = await ModelDB.connect_to_db()
    debug('counting number of times seq is used')
    try{
      const sql = "SELECT count(*) FROM " + this.dbName + " WHERE seq_id = $1"
      const res = await client.query(sql, [seq_id])
      debug("sequence used in " + res.rows[0].count + " sessions")
      return res.rows[0].count
    }
    catch(err){
      debug("ERROR verifying if used sequence")
      return false
    }
    finally {
      client.end()
