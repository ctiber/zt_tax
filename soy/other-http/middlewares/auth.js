...
const tracer = trace.getTracer('soy-ms-other');
    ...
    if ( req.headers.cookie ) {
      ...
      cookies.forEach( ( cookie ) => {
        ...
        if ( key === "access_token" ) {
          ...
          const authSpan = tracer.startSpan('zt.ac4a.verify');
          try {
            ...
            authSpan.setAttribute('error.type', err.name);
            authSpan.setStatus({ code: SpanStatusCode.ERROR, message: err.name });
            authSpan.end();
