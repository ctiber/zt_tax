...
class ModelSkill extends Model {
  ...
  static async readThemes(locale){
    debug("reading all themes")
    const client = ModelDB.connect_to_db();
    try {
      const sql =
        "SELECT * FROM theme WHERE locale = $1;";
      const res = await client.query(sql, [locale]);
      debug("Read themes : SUCCESS");
      return res.rows;
    } catch (err) {
      debug("Read themes : ERROR " + err.stack);
      return false;
    } finally {
      client.end();
    }
